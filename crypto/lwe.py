...
.. [LPR10] Vadim Lyubashevsky, Chris Peikert, and Oded Regev. On Ideal Lattices
   and Learning with Errors over Rings. in Advances in Cryptology â€“ EUROCRYPT
   2010. Springer 2010. http://dx.doi.org/10.1007/978-3-642-13190-5_1

.. [CGW13] Daniel Cabarcas, Florian GÃ¶pfert, and Patrick Weiden. Provably Secure
...
class RingLWE(SageObject):
    ...
    def __call__(self):
        ...
        EXAMPLE::
            ...
            sage: N = 16
            sage: n = euler_phi(N)
            sage: D = DiscreteGaussianPolynomialSamplerRejection(n, 5)
...
class RingLindnerPeikert(RingLWE):
    ...
    def __init__(self, N, delta=0.01, m=None):
        ...
        EXAMPLES::
            ...
            RingLWE(16, 1031, DiscreteGaussianPolynomialSamplerRejection(8, 2.803372, 53, 4), x^8 + 1, 'noise', None)
        """
        if m is None:
            m = 3*n
        n = euler_phi(N)
...
class RingLWEConverter(SageObject):
    ...
    def _repr_(self):
        ...
        EXAMPLE::
            ...
            sage: D = DiscreteGaussianPolynomialSamplerRejection(euler_phi(20), 5)
